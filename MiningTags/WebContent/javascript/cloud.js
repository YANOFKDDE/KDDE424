  ...
  text.enter().append("text")
	  ...
	  .on("click",function(d){
		//在每一个匹配元素的mouseover事件中绑定一个处理函数。
			$("#relevance").val("");
			var word = d.text.trim();
			for (var i = 0; i < miningResult.relevancelist.length; i++) {
				var labellist = miningResult.relevancelist[i];
				for (var j = 0; j < labellist.length; j++) {
					var labels = labellist[j];
					if (labels.match(word)) {
						$("#relevance").val( $("#relevance").val() + labels + "\n");
					}
				}
			}
			if ($("#relevance").val().trim() == "") {
				alert("没有相应的关联规则！");
			}
		...
		$("text").each(function(i){
			...
			$(this).mouseover(function(){//在每一个匹配元素的mouseover事件中绑定一个处理函数。
					...
					$("text").each(function(i){
						...
						var text = $(this).text();
						if(!highlights[highlightkey].match(text)){
